...
const Contact = ({ title, description, buttonText, buttonLink, variant = 'primary' }) => {
  ...
  const buttonVariants = {
    primary:
      'bg-gradient-to-r from-secondary to-secondary-dark text-white hover:from-secondary-light hover:to-secondary shadow-lg hover:shadow-secondary/40',
    secondary: 'bg-text-primary text-primary hover:bg-text-secondary shadow-lg',
  };

  return (
    <section className={`py-20 md:py-24 ${variants[variant] || variants.primary}`}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
        ...
        <p className="text-base sm:text-lg md:text-xl mb-10 opacity-90 max-w-2xl mx-auto leading-relaxed">
          {description}
        </p>
        {buttonText && buttonLink && (
          <Link
            to={buttonLink}
            className={`inline-flex items-center px-8 py-4 font-semibold rounded-full transition-all duration-300 transform hover:scale-105 ${buttonVariants[variant] || buttonVariants.primary}`}
          >
            {buttonText}
